...
public interface NewsDAO {
    ...
    String INSERT_FIELDS = " title, content, image, like_count, comment_count, created_date, user_id";
    String SELECT_FIELDS = " id, " + INSERT_FIELDS;
    @Select({"SELECT", SELECT_FIELDS, " FROM ", TABLE_NAME, " WHERE id=#{id}"})
    News selectById(@Param("id") int id);

    @Insert({"INSERT INTO ", TABLE_NAME, "(", INSERT_FIELDS, ") VALUES(#{title}, #{content},#{image}, #{likeCount}, #{commentCount}, #{createdDate}, #{userId})"})
